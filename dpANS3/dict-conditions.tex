...
\EV HANDLE-DIVISION-CONDITIONS
 (defun handle-other-arithmetic-errors (condition)
   (format t "Considering condition for arithmetic condition handling~%")
   (when (typep condition 'arithmetic-error)
     (abort)))
\EV HANDLE-OTHER-ARITHMETIC-ERRORS
 (define-condition a-condition-with-no-handler (condition) ())
\EV A-CONDITION-WITH-NO-HANDLER
 (signal 'a-condition-with-no-handler)
\EV NIL
 (handler-bind ((condition #'handle-division-conditions)
                  (condition #'handle-other-arithmetic-errors))
   (signal 'a-condition-with-no-handler))
\OUT Considering condition for division condition handling
\OUT Considering condition for arithmetic condition handling
\EV NIL
