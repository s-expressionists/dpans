...
order. The modification order is a sequence of \term{lock
  acquisitions} followed by \term{lock releases}. The modification
order may be empty if no \term{thread} ever acquires the lock, but
otherwise, it begins with a \term{lock acquisition}. A \term{lock}'s
modification order is the same between all \term{threads}.

A \term{lock acquisition} specifically occurs when a call to
\funref{acquire-lock} on that lock returns \term{true}, and a
...
For any \term{lock release} in the modification order, the \term{lock
  acquisition} immediately before it in the order is
\term{sequenced-before} the \term{lock release}, because only the
\term{thread} that acquired a lock can release it. It is further
defined that for any \term{lock acquisition} of the lock past the
first, the \term{lock release} immediately before it in the order
\term{synchronizes-with} the \term{lock acquisition}. Together, these
two properties ensure that any operation in the modification order
\term{happens-before} the subsequent operation in the modification
order, and therefore \term{happens-before} all subsequent operations
in the modification order.

The modification orders of locks must result in a consistent
(i.e. acyclic) \term{happens-before} ordering of
