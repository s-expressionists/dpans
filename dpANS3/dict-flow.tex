...
exits normally before the \specref{return-from} \term{form} is attempted.
...
by the \specref{catch}), while \specref{block} and \macref{return} are used
...
Otherwise, if the \param{test-form} \term{yields} \term{true},
the \param{forms} are not \term{evaluated},
...
%% 7.9.1 12
\specref{multiple-value-prog1} evaluates \param{first-form} and saves
...
Operationally, the following relationship is true, although \macref{nth-value}
...
\macref{prog2} \term{evaluates} \param{first-form},
 ...
 and then \param{forms},
\term{yielding} as its only \term{value}
the \term{primary value} \term{yielded} by \param{second-form}.
...
that accepts \keyref{optional}, \keyref{rest},
   or \keyref{key} arguments and that 
